...
class IssuesController < ApplicationController
  menu_item :new_issue, :only => [:new, :create, :update_valor_peso]
  ...
  def create
    ...
    if @issue.save
      ...
      #if @issue.tracker_id != 9 || @issue.tracker_id != 11 || @issue.tracker_id != 8
        padre = Issue.find(@issue.parent_issue_id)
        @issues = Issue.where(parent_id: padre.id)
        total = 0
        unless @issue.tracker_id == 9 || @issue.tracker_id == 11 || @issue.tracker_id == 8
          @issues.each do |iss|
            total += iss.valor_peso
          end
        end
        if Issue.where(parent_id: Issue.where(tracker_id: 9).last.id).count < 3
          ...
          respond_to do |format|
            format.html {
              ...
              if @issues.count == 1
                unless @issue.tracker_id == 9 || @issue.tracker_id == 11 || @issue.tracker_id == 8
                  flash[:notice] = l(:notice_set_100_percent)
                end
              elsif @issues.count == 2
                unless @issue.tracker_id == 9 || @issue.tracker_id == 11 || @issue.tracker_id == 8
                  flash[:notice] = l(:notice_2_issues_updated)
                end
              else
                unless @issue.tracker_id == 9 || @issue.tracker_id == 11 || @issue.tracker_id == 8
          ...
          #redirect_to '/issues/update_valor_peso'
          #redirect_to :action => "update_valor_peso"
          #format.api { render :action => 'update_valor_peso', :layout => !request.xhr? }
          render "update_valor_peso"
        else
          redirect_to issue_path(@issue)
        end
      else
  ...
  def update
    return unless update_issue_from_params
    if @issue.tracker_id==5
      valor_anterior = @issue.valor_peso
      ...
      if @issues.count == 1
        @issue.valor_peso = 100
        #flash[:notice] = l(:notice_set_100_percent)
      elsif @issues.count == 2
        if @issue.id == @issues.first.id
          valor_nuevo = 100 - @issue.valor_peso
          Issue.where(parent_id: padre.id).last.update_column(:valor_peso, valor_nuevo)
        elsif @issue.id == @issues.last.id
          valor_nuevo = 100 - @issue.valor_peso
          Issue.where(parent_id: padre.id).first.update_column(:valor_peso, valor_nuevo)
        end
        #flash[:notice] = l(:notice_2_issues_updated)
      end
    end
    ...
    if @issue.tracker_id==5
      ...
      if @issue.tracker_id == 5
        if total == 100
          if saved
            ...
            if @issues.count == 1
              ...
              if valor_anterior != @issue.valor_peso
                flash[:notice] = l(:notice_2_issues_updated)
              end
            else
        ...
        #format.api  { render :action => 'show', :status => :updated, :location => issue_url(@issue)}
        respond_to do |format|
          format.html { redirect_back_or_default issue_path(@issue) }
          format.api  { render_api_ok }
        end
