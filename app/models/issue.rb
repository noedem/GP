...
class Issue < ActiveRecord::Base
  ...
  #after_save :validate_valor_peso_suma

  def set_valor_peso
    #binding.pry
    #padre = Issue.where(tracker_id: 9).last
    #padre = Issue.find(self.parent_id)
    if self.tracker_id == 9 || self.tracker_id == 11 || self.tracker_id == 8
      self.valor_peso = nil
    elsif Issue.where(parent_id: parent_issue_id).count == 0
      self.valor_peso = 100
    elsif Issue.where(parent_id: parent_issue_id).count == 1
      #binding.pry
      #ind2 = Issue.where(parent_id: padre.id).last
      if self.valor_peso == nil
        self.valor_peso = 0
      end
      valor_nuevo = 100 - self.valor_peso
      ind1 = Issue.where(parent_id: parent_issue_id).first
      if ind1.id != self.id
